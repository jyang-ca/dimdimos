...
def _run_simulation(config: GlobalConfig, shm: ShmReader) -> None:
    ...
    # Offscreen renderers feed color_image/lidar streams, which Rerun logs.
    ...
    def step_simulation(m_viewer: Any | None = None) -> bool:
    ...
    renderers = [rgb_renderer, depth_renderer, depth_left_renderer, depth_right_renderer]

    try:
        shm.signal_ready()

        if config.mujoco_native_viewer:
            ...
            with viewer.launch_passive(
                ...
                while not shm.should_stop() and step_simulation(m_viewer):
                    pass
        else:
            logger.info("Running MuJoCo with offscreen rendering only")
